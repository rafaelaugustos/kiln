...
func (p *recent) pick(rng *rand.Rand) int64 {
	p.mu.Lock()
	defer p.mu.Unlock()
	if len(p.ids) == 0 {
		return 0
...
func TestChurn(t *testing.T) {
	...
	drain(t, s)
	if n := count(t, s, "SELECT COALESCE(SUM(active), 0) FROM kiln_limits"); n != 0 {
		t.Fatalf("limits still count %d active slots after draining", n)
	}
	t.Logf("inserted %d, finished %d outcomes", inserted.Load(), finished.Load())
}
