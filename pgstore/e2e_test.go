...
func newCluster(tb testing.TB) *cluster {
	tb.Helper()
	pool := pgstore.Connect(tb)
	c := &cluster{tb: tb, pool: pool, schema: fmt.Sprintf("ke_%x", rand.Uint64())}
	tb.Cleanup(func() {
		if _, err := pool.Exec(context.Background(), "DROP SCHEMA IF EXISTS "+c.schema+" CASCADE"); err != nil {
			tb.Errorf("drop schema: %v", err)
		}
		pool.Close()
	})
	return c
...
func (c *cluster) store(opts ...pgstore.Option) *pgstore.Store {
	c.tb.Helper()
	s, err := pgstore.New(context.Background(), c.pool, append([]pgstore.Option{pgstore.Schema(c.schema)}, opts...)...)
	if err != nil {
		c.tb.Fatal(err)
	}
...
func holding(started chan<- int64, causes chan<- error) *kiln.Mux {
	m := kiln.NewMux()
	kiln.Handle(m, func(ctx context.Context, j *kiln.Job[hold]) error {
		send(started, j.ID)
		<-ctx.Done()
		send(causes, context.Cause(ctx))
		return ctx.Err()
	})
	return m
}
...
func TestE2EContinuations(t *testing.T) {
	t.Parallel()
	st := newCluster(t).store()
	tr := &trace{}
...
func TestE2EDeleteCancelsRunning(t *testing.T) {
	t.Parallel()
	c := newCluster(t)
	started := make(chan int64, 1)
	causes := make(chan error, 1)
	for range 2 {
		serve(t, c.store(), holding(started, causes), fast())
	}
	cl := kiln.NewClient(c.store())
	ctx := context.Background()
	id := enqueue(t, cl, hold{})
	if got := recv(t, started, 5*time.Second); got != id {
		t.Fatalf("started %d, want %d", got, id)
	}
	n, err := cl.Delete(ctx, id)
	if err != nil || n != 1 {
		t.Fatalf("delete: %d, %v", n, err)
	}
	if cause := recv(t, causes, 5*time.Second); !errors.Is(cause, kiln.ErrCanceled) {
		t.Fatalf("handler cause %v", cause)
	}
	r := waitState(t, cl, id, kiln.Deleted)
	if rs := reasons(r); len(rs) == 0 || rs[len(rs)-1] != "canceled" {
...
func TestE2EOrphanRescue(t *testing.T) {
	...
	srvA, _ := serve(t, a, holding(started, nil), cfg)
	cl := kiln.NewClient(c.store())
...
func TestE2EShutdownRequeues(t *testing.T) {
	t.Parallel()
	st := newCluster(t).store()
	started := make(chan int64, 1)
	causes := make(chan error, 1)
	...
	_, stop := serve(t, st, holding(started, causes), cfg)
	cl := kiln.NewClient(st)
	id := enqueue(t, cl, hold{}, kiln.MaxAttempts(1))
	recv(t, started, 5*time.Second)
	stop()
	if cause := recv(t, causes, time.Second); !errors.Is(cause, kiln.ErrShutdown) {
		t.Fatalf("handler cause %v", cause)
	}
	r := get(t, cl, id)
	if r.State != kiln.Enqueued || r.Attempt != 0 {
		t.Fatalf("after shutdown: state %s attempt %d", r.State, r.Attempt)
	}
	if rs := reasons(r); len(rs) == 0 || rs[len(rs)-1] != "shutdown" {
		t.Fatalf("reasons %v", rs)
	}

	again := make(chan *kiln.Job[hold], 1)
	m := kiln.NewMux()
	kiln.Handle(m, func(_ context.Context, j *kiln.Job[hold]) error {
		send(again, j)
		return nil
	...
	serve(t, st, m, fast())
	if j := recv(t, again, 5*time.Second); j.ID != id || j.Attempt != 1 {
		t.Fatalf("rerun job %d attempt %d", j.ID, j.Attempt)
	}
