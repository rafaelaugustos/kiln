...
func encodeMeta(m map[string]string) jsonText {
	if len(m) == 0 {
		return nil
	}
	b := make([]byte, 0, 64)
...
func encodeStrings(ss []string) jsonText {
	if ss == nil {
		return nil
	}
...
func encodeIDs(ids []int64) jsonText {
	if ids == nil {
		return nil
	}
...
func (e entry) encode(at time.Time) jsonText {
	if e.reason == "" {
		return nil
	}
...
func (s *stamp) Scan(v any) error {
	...
	case nil:
		...
		return nil
	case time.Time:
		s.Time = time.Date(v.Year(), v.Month(), v.Day(), v.Hour(), v.Minute(), v.Second(), v.Nanosecond(), time.UTC)
		return nil
	case []byte:
		return s.parse(v)
	case string:
		return s.parse([]byte(v))
	}
	return fmt.Errorf("kiln: cannot scan %T into a timestamp", v)
}

func (s *stamp) parse(b []byte) error {
	if len(b) < 19 || b[4] != '-' || b[7] != '-' || b[10] != ' ' || b[13] != ':' || b[16] != ':' {
		return fmt.Errorf("kiln: bad timestamp %q", b)
	}
	...
	nsec := 0
	if len(b) > 20 && b[19] == '.' && len(b) <= 29 {
		nsec = num(20, len(b))
		for range 29 - len(b) {
			nsec *= 10
		}
	}
	if year < 0 || month < 0 || day < 0 || hour < 0 || minute < 0 || second < 0 || nsec < 0 {
		return fmt.Errorf("kiln: bad timestamp %q", b)
	}
	s.Time = time.Date(year, time.Month(month), day, hour, minute, second, nsec, time.UTC)
	return nil
}
