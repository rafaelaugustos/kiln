...
func newCluster(tb testing.TB) *cluster {
	tb.Helper()
	return &cluster{tb: tb, db: mysqlstore.Database(tb)}
}

func (c *cluster) store(opts ...mysqlstore.Option) *mysqlstore.Store {
	c.tb.Helper()
	s, err := mysqlstore.New(context.Background(), c.db, opts...)
	if err != nil {
		c.tb.Fatal(err)
	}
...
func TestE2ETxEnqueue(t *testing.T) {
	...
	id, err := cl.EnqueueTx(ctx, st.Tx(tx), double{N: 2})
	if err != nil {
		t.Fatal(err)
	}
